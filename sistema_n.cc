...
ModuleInterface(sistema)
    // parameters:
    Parameter(tamTrama, ParType_Numeric)
EndInterface

Register_ModuleInterface(sistema);

class sistema : public cCompoundModule
{
  public:
    sistema() : cCompoundModule() {}
  protected:
    virtual void doBuildInside();
};

Define_Module(sistema);

void sistema::doBuildInside()
{
    cModule *mod = this;
    ...
    mod->setBackgroundDisplayString("o=yellow,,;b=274,179,,,,");
    ...
    cModule *Host_1_p = modtype->create("Host_1", mod);
    {
        cContextSwitcher __ctx(Host_1_p); // do the rest in this module's context

        // parameter assignments:
        Host_1_p->par("tamTrama") = mod->par("tamTrama");
        Host_1_p->par("direccion") = 1;

        _readModuleParameters(Host_1_p);
        Host_1_p->setDisplayString("p=71,79;i=old/server1");
    }
    ...
    int Host_2_size = 1;
    modtype = _getModuleType("host");
    cModule *Host_2_p = modtype->create("Host_2", mod);
    {
        cContextSwitcher __ctx(Host_2_p); // do the rest in this module's context

        // parameter assignments:
        Host_2_p->par("tamTrama") = mod->par("tamTrama");
        Host_2_p->par("direccion") = 2;

        _readModuleParameters(Host_2_p);
        Host_2_p->setDisplayString("p=190,79;i=old/server1");
    }
    ...
    cPar *par;
    // connection
    srcgate = _checkGate(Host_1_p, "salida");
    destgate = _checkGate(Host_2_p, "entrada");
    channel = _createChannel("canal");
    srcgate->connectTo(destgate,channel);

    // connection
    srcgate = _checkGate(Host_2_p, "salida");
    destgate = _checkGate(Host_1_p, "entrada");
    channel = _createChannel("canal");
    srcgate->connectTo(destgate,channel);
    ...
    //
    Host_1_p->buildInside();
    Host_2_p->buildInside();
}

// channel definition canal
class canal : public cChannelType
{
  public:
    canal(const char *name) : cChannelType(name) {}
    canal(const canal& n) : cChannelType(n.name()) {operator=(n);}
    virtual cChannel *create(const char *name);
};

Define_Channel(canal);

cChannel *canal::create(const char *name)
{
    cChannel *chan = new cBasicChannel(name);
    cPar tmpval, *p;
...
class red : public cNetworkType
{
  public:
    red(const char *name) : cNetworkType(name) {}
    red(const red& n) : cNetworkType(n.name()) {operator=(n);}
    virtual void setupNetwork();
};

Define_Network(red);

void red::setupNetwork()
{
    // temporary variables:
    cPar tmpval;
    const char *modtypename;
    ...
    modtype = _getModuleType("sistema");
    cModule *red_p = modtype->create("red", NULL);

    cContextSwitcher __ctx(red_p); // do the rest in this module's context
    // parameter assignments:
    red_p->par("tamTrama") = (tmpval=4, tmpval.setPrompt("Ingrese el largo de la palabra, entero mayor a cero:"), tmpval.setInput(true), tmpval);

    _readModuleParameters(red_p);
